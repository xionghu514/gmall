...
2. 定义一个静态内部实体类，定义接受参数的字段(如果要接受不定数量的参数，可以定义集合字段)
3. 继承AbstractGatewayFilterFactory抽象类时，指定泛型step2定义的实体类
4. 重写父类的无参构造方法，调用super(step3中定义的实体类.class)
5. 重写shortcutFieldOrder()方法指定接受参数的字段顺序
6. 如果要使用集合字段接受参数需要重写shortcutType方法，返回ShortcutType.GATHER_LIST
7. 配置局部过滤器

## 局部过滤器实现统一认证

1. 判断当前请求是否在拦截名单中, 不在折直接放行
